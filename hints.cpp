...
public:
	...
	{		
		if (Collections::verify_item("hints").Name != "")
		{
		...
		else
		{
			...
			const string itemOrder[11] = { "tool belt", "water bottle", "key", "knife", "ladder", "lantern", "batteries", "metal detector", "shovel", "dentures", "mouthguard" };
			vector<Items> missing = {};
			int firstMissing = -1;

			for (string name : itemOrder)
			{
				if (!Collections::verify_inventory(name)) {
					missing.push_back(Collections::verify_item(name));
				}
			}

			firstMissing = distance(itemOrder->begin(), find(itemOrder->begin(), itemOrder->end(), missing[0]));

			if (firstMissing >= 0)
			{
				Dialogue::print_line("Get " + itemOrder[firstMissing]);
			}
			else
			{
			...
			case 3: //knife in kitchen
				if (Collections::verify_room("Garage").Locked)
				{
			...
			case 4: //ladder in garage
				...
				else if (Characters_Handler::player.get_experience() < 10)
				{
					Dialogue::print_line("You need " + to_string(10 - Characters_Handler::player.get_experience()) + " more sticks. The PANTRY is full of lolipops. Go start some trouble there for some extra sticks.");
				}
				else
				{
			...
			case 5: //lantern in attic
				if (Collections::verify_room("Attic").Locked)
				{
			...
			case 7: //metal detector in basement
				if (Collections::verify_room("Basement").Locked)
				{
			...
			case 8: //shovel in garage
				if (Collections::verify_room("Backyard").Usable_items.find("shovel") != Collections::verify_room("Backyard").Usable_items.end())
				{
			...
			default: //got all items except maybe mints
				if (Collections::verify_room("Hidden Room").Name != "")
				{
